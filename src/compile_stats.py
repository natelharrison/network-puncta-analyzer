...
def assign_comparison_group(trial_name, strain):
    ...
    if 'R14' in trial_clean:
        if strain in ['Rosa 1', 'V1']: return "R14 (Rosa1 vs V1)"
        if strain in ['Rosa 2', 'V2']: return "R14 (Rosa2 vs V2)"
    if 'R13' in trial_clean: return "R13 (Rosa vs V1)"
    if 'R15' in trial_clean: return "R15 (BL6 vs V1)"
    return trial_name
...
def plot_metric_polished(ax, data, x_col, y_col, hue_col, title, ylabel, show_legend=False):
    """
    Academic Style Plot: Strip Plot (Raw Data) + Point Plot (Mean +/- Error)
    """
    # Colors: Dark Grey for WT, Distinct Red for Mutant
    colors = {"WT": "#444444", "Mutant": "#D62828"}
    order = sorted(data[x_col].unique())
    hue_order = ["WT", "Mutant"]

    # 1. Reference Line for Normalized Data
    if 'Norm' in title:
        ax.axhline(1.0, linestyle=':', color='black', alpha=0.4, linewidth=1)

    # 2. Raw Data Points (Strip Plot)
    sns.stripplot(
    ...
    # 3. Mean & Error Bars (Point Plot)
    # Join=False removes the connecting line between groups
    # Capsize adds the little T-bars on errors
    sns.pointplot(
        data=data, x=x_col, y=y_col, hue=hue_col,
        palette=colors, order=order, hue_order=hue_order,
        dodge=0.4, join=False, capsize=0.1, errorbar='se',
        markers="D", scale=0.8, ax=ax, legend=False, zorder=10
    )
    ...
    for i, group in enumerate(order):
        ...
        if len(wt) > 1 and len(mut) > 1:
            ...
            # Calculate clear position for text
            y_max = sub[y_col].max()
            y_pos = y_max + (y_max * 0.1) if y_max > 0 else 1.1

            # Draw bracket or just text? Text is cleaner for high throughput.
            ax.text(i, y_pos, f"{sig}\n(p={p:.3f})", ha='center', va='bottom', fontsize=9, fontweight='bold')

    # 6. Legend (Custom to ensure it picks up the Point Plot colors)
    if show_legend:
...
def make_nice_plots(net_df, puncta_df, output_dir):
    sns.set_theme(style="ticks", context="talk")  # Clean white background with ticks
    ...
    if puncta_df is not None:
        ...
        plots_to_make.append((puncta_df, 'avg_nnd_norm', 'Puncta Spacing (Norm)', 'Fold Change vs WT'))
    ...
    sns.despine(offset=10, trim=True)  # Clean look
    plt.tight_layout()
    ...
    for i, (data, col, title, ylab) in enumerate(plots_to_make):
        # For global, x-axis is just Genotype
...
def generate_montages(data_dir):
    print("\n--- Generating Montages ---")
    all_overlays = list(data_dir.rglob("*/analysis_results/network_overlay/*.tif"))
    if not all_overlays: return
    trials = {}
    for p in all_overlays:
        trial_name = p.parents[2].name
        if trial_name not in trials: trials[trial_name] = []
        trials[trial_name].append(p)
    for trial_name, paths in tqdm(trials.items(), desc="Building"):
        output_dir = paths[0].parents[1]
        pipeline_utils.create_montage(trial_name, paths, output_dir / "Montage_Network.png")
...
def main():
    ...
    if net_df is not None:
        ...
        if 'tissue_area' in net_df.columns: net_df.rename(columns={'tissue_area': 'mask_area'}, inplace=True)
        cols = ['image_name', 'trial', 'total_ridge_pixels_norm', 'total_ridge_pixels', 'object_count',
                'structure_index', 'mask_area', 'strain', 'genotype', 'comparison_group']
        net_df[[c for c in cols if c in net_df.columns]].to_csv(args.data_dir / "DATA_Networks_Combined.csv",
                                                                index=False)
    ...
    if puncta_df is not None:
        ...
        if 'tissue_area' in puncta_df.columns: puncta_df.rename(columns={'tissue_area': 'mask_area'}, inplace=True)
        cols = ['image_name', 'trial', 'puncta_count_norm', 'puncta_count', 'puncta_density_norm', 'puncta_density',
                'avg_nnd_norm', 'avg_nnd', 'mask_area', 'strain', 'genotype', 'comparison_group']
        puncta_df[[c for c in cols if c in puncta_df.columns]].to_csv(args.data_dir / "DATA_Puncta_Combined.csv",
                                                                      index=False)

    make_nice_plots(net_df, puncta_df, args.data_dir)
    generate_montages(args.data_dir)
